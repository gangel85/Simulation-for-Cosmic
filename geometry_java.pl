...
#	my @allMeshes =("OpticalGasVolume", "Aluminum","AerogelTiles","CFRP","Glass","TedlarWrapping","SphericalMirrors");
	my @allMeshes =("Aluminum","OpticalGasVolume");
...
sub build_Aerogel{
  my $aerogel_tiles = 1;
  for (my $iaerogel=0; $iaerogel<$aerogel_tiles; $iaerogel ++)
  {
  my $vname = sprintf("Aerogel_${iaerogel}");
  my %detector = init_det();
    %detector = init_det();
  $detector{"name"}        = "$vname";
  $detector{"mother"}      = $mothers->{$vname};
  $detector{"description"} = "Aerogel";
  $detector{"pos"}         = $positions->{$vname};
  $detector{"rotation"}    = $rotations->{$vname};
  $detector{"color"}       = "444444";
...
sub build_TopPmts{
  ...
  for (my $ipmt=0; $ipmt<$topPmts; $ipmt ++)
  {
    my $ipmt2=400+$ipmt;
    
    my $vname = sprintf("MAPMT_${ipmt2}");
    my %detector = init_det();
    ...
    $detector{"identifiers"} = "sector manual 4 pad manual $ipmt2 pixel manual 1";
    print_det(\%main::configuration, \%detector);
...
sub build_Scintillator{
  
  my $vname = sprintf("Scintillator");
  my %detector = init_det();
